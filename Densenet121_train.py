...
# 定义缓存目录
root_dir = "/work/home/ac609hjkef/liyp/Direct_classification_2/Dense121"
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            if metric > best_metric:
                ...
                torch.save(
                    model.state_dict(),
                    "best_metric_model_Densenet121.pth",
                )
            print(
...
# 测试阶段
model.load_state_dict(torch.load("best_metric_model_Densenet121.pth"))
model.eval()
with torch.no_grad():
    num_correct = 0.0
    metric_count = 0
